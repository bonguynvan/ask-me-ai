...
<template>
  <article class="grid grid-cols-1 lg:grid-cols-2 gap-x-8">
    <div class="mt4">
      <h3>Enter the text you would like to ask question about</h3>
      <section>
        <textarea
          rows="20"
          v-model="textChatStore.text"
          class="block w-full rounded-md border-0 text-gray-100 shadow-sm 
          ring-1 ring-inset rign-gray-300 placeholder:text-gray-400 sm:p-1.5 sm:text-sm sm:leading-6 text-sm my-4"
        />
      </section>
    </div>
    <div>
      ...
      <div>
        <div class="flex rounded-md shadow-sm mt-4">
          ...
          <button 
            type="button"
            @click="sendQuestion"
            class="chat-btn group relative -ml-px inline-flex items-center gap-x-1.5 rounded-r-md px-3 text-sm font-semibold bg-[#1a1a21] text-green-500 ring-1 ring-inset ring-gray-300"
          >
